...
int array_dot_vec(const int *a, const int *b, int len) {
    // Allocate aligned memory for temp array
    int *temp = alloc_aligned_int_array(len);

    // Initialize the sum and a vector to store intermediate results
    int sum = 0;
    __m256i sum_vec = _mm256_setzero_si256();

    // Perform vectorized dot product on 256-bit chunks
    for(int i = 0; i <= len - 8; i += 8){
        // Load 256 bits of data from arrays 'a' and 'b' into AVX registers
    ...
    // Horizontal sum of the elements in the vector
    int temp_arr[8] __attribute__((aligned(32)));
    _mm256_store_si256((__m256i *)temp_arr, sum_vec);
    for (int i = 0; i < 8; i++){
        sum += temp_arr[i];
    }

    // Calculate the remaing elements
