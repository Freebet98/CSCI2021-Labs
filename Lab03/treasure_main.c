...
treasuremap_t *treasuremap_load_text(char *file_name) {
    ...
    //Check if the file fails to open and return NULL if so.
    if (file_handle == NULL) {
    ...
    //Determine byte size for treasuremap_t struct
    ...
    for (int i = 0; i < tmap->ntreasures; i++) {
        ...
        //read in the column location for this treasure
        fscanf(file_handle, "%d", &tmap->locations[i].col);

        //read in the description for this treasure
...
treasuremap_t *treasuremap_load_binary(char *file_name) {
    ...
    FILE *file_handle = fopen(file_name, "r");
    //check for failure and return appropriate value
    if (file_handle == NULL) {
        printf("Couldn't open file '%s', returning NULL\n", file_name);
        return NULL;
    }
    ...
    printf("Allocating map struct\n");
    //How much space to allocate for treasuremap_t struct?
    treasuremap_t *tmap = malloc(sizeof(treasuremap_t));
    //Determine how many bytes to read for treasure map height
    fread(&tmap->height, sizeof(int), 1, file_handle);
    //Determine how many int elements to read in for treasure map width
    ...
    printf("Allocating array of treasure locations\n");
    //Allocate space to hold all treasure locations
    tmap->locations = malloc(tmap->ntreasures * sizeof(treasureloc_t));

    printf("Reading treasures\n");
    //Read in each treasure from the file
    for (int i = 0; i < tmap->ntreasures; i++) {
        //Read in treasure location's row
        fread(&tmap->locations[i].row, sizeof(int), 1, file_handle);
        fread(&tmap->locations[i].col, sizeof(int), 1, file_handle);

        int description_len;
        //Read in the length of the treasure's description
        fread(&description_len, sizeof(int), 1, file_handle);
        //Knowing this length, read in the full treasure description
...
// PROVIDED AND COMPLETE: Prffint out the treasure map; uses local
